...
async def mcp(ctx, *, query: str):
    ...
    try:
        ...
        all_text = " ".join(p.get_text(strip=True) for p in paragraphs if len(p.get_text(strip=True)) > 30)

        if len(all_text) < 200:
            await ctx.send("⚠️ 抓到的資料可能不足，AI 仍將嘗試統整。")

        if len(all_text) > 12000:
            all_text = all_text[:12000]
            await ctx.send("⚠️ 文章過長，已截取前 12000 字。")

        # Step 4: 請 AI 分析你提供的內容（不告訴它網址）
        prompt = (
